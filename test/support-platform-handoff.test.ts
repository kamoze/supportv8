...
describe("Sophia platform launch", () => {
  it("sends an unhired SupportV8 tenant to Marketplace with canonical Registry scope", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
      ...
      if (url.endsWith("/v1/tenants/by-slug/acme")) {
        return Response.json({ id: "tenant_acme", accountId: "account-acme", slug: "acme", status: "active" });
      }
      if (url.includes("/v1/tenants/tenant_acme/memberships/user-1")) {
        return Response.json({ tenantId: "tenant_acme", accountId: "account-acme", identitySubject: "user-1", status: "active" });
      }
    ...
    expect(requests.some((url) => url.includes("accountId=account-acme") && url.includes("tenantId=tenant_acme") && url.includes("verticalId=supportv8"))).toBe(true);
  ...
  it("sends an exact active Sophia hire to Studio configuration", async () => {
    vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
      ...
      if (url.endsWith("/v1/tenants/by-slug/acme")) return Response.json({ id: "tenant_acme", accountId: "account-acme", slug: "acme", status: "active" });
      if (url.includes("/memberships/user-1")) return Response.json({ tenantId: "tenant_acme", accountId: "account-acme", identitySubject: "user-1", status: "active" });
      if (url.includes("/v1/projections/installations")) return Response.json({ installations: [
        { accountId: "account-other", tenantId: "tenant_acme", verticalId: "supportv8", productId: "servicev8.ai-support-agent", entitlementStatus: "active", installationId: "wrong", hireId: "wrong" },
        { accountId: "account-acme", tenantId: "tenant_acme", verticalId: "supportv8", productId: "servicev8.ai-support-agent", entitlementStatus: "active", installationId: "installation-sophia", hireId: "hire-sophia" },
      ] });
      return new Response(null, { status: 404 });
    }));
  ...
  it("fails closed when Registry identity scope disagrees with SupportV8", async () => {
    vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
      ...
      if (url.endsWith("/v1/tenants/by-slug/acme")) return Response.json({ id: "tenant_other", accountId: "account-acme", slug: "acme", status: "active" });
      return new Response(null, { status: 404 });
    }));
