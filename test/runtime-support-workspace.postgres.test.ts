...
describe.skipIf(!enabled)("runtime Support workspace genuine PostgreSQL boundary", () => {
  ...
  beforeAll(async () => {
    ...
    const emailMigration=await readFile(new URL("../migrations/004_email_channel_bindings.sql",import.meta.url),"utf8");
    const migration=await readFile(new URL("../migrations/005_runtime_support_workspaces.sql",import.meta.url),"utf8");
    await admin.query("SET ROLE support_app"); await admin.query(emailMigration); await admin.query(migration); await admin.query("RESET ROLE");
    await admin.query("GRANT USAGE ON SCHEMA supportv8 TO support_reader; GRANT SELECT,UPDATE,DELETE ON supportv8.runtime_support_workspaces TO support_reader");
  ...
  it("rejects changed scope, operation reuse, native/domain collision, rollback retry, and tombstone resurrection",async()=>{
    ...
    await app.query("DELETE FROM supportv8.tenants WHERE id='tenant_collision'");
    expect(await store.acquire(base("collision"))).toMatchObject({status:"workspace_created",domain:"collision.support.test"});
    ...
    const tombstoneClient=await app.connect();try{await tombstoneClient.query("BEGIN");await tombstoneClient.query("SELECT set_config('app.current_tenant_id',$1,true),set_config('app.current_account_id',$2,true),set_config('app.current_registry_tenant_id',$3,true)",[made.workspaceId,input.accountId,input.registryTenantId]);await expect(tombstoneClient.query("UPDATE supportv8.runtime_support_workspaces SET state='workspace_created',deleted_at=NULL WHERE installation_id=$1",[input.installationId])).rejects.toThrow(/invalid runtime support workspace state transition/);await tombstoneClient.query("ROLLBACK");}finally{tombstoneClient.release();}
  });
  it("enforces restricted-role RLS and immutable identity guards",async()=>{
    ...
    const scoped=async(account:string|undefined,registry:string|undefined,sql:string,tenantId=made.workspaceId)=>{const c=await reader.connect();try{await c.query("BEGIN");if(tenantId)await c.query("SELECT set_config('app.current_tenant_id',$1,true)",[tenantId]);if(account)await c.query("SELECT set_config('app.current_account_id',$1,true)",[account]);if(registry)await c.query("SELECT set_config('app.current_registry_tenant_id',$1,true)",[registry]);const r=await c.query(sql);await c.query("ROLLBACK");return r;}catch(error){await c.query("ROLLBACK").catch(()=>undefined);throw error;}finally{c.release();}};
    ...
    expect((await scoped(input.accountId,input.registryTenantId,"SELECT * FROM supportv8.runtime_support_workspaces","tenant_wrong_native")).rowCount).toBe(0);
    expect((await scoped(undefined,undefined,"SELECT * FROM supportv8.runtime_support_workspaces","")).rowCount).toBe(0);
    expect((await scoped("wrong",input.registryTenantId,"UPDATE supportv8.runtime_support_workspaces SET updated_at=now()")).rowCount).toBe(0);
    expect((await scoped(input.accountId,"wrong","UPDATE supportv8.runtime_support_workspaces SET updated_at=now()")).rowCount).toBe(0);
    ...
    await expect(scoped(input.accountId,input.registryTenantId,"UPDATE supportv8.runtime_support_workspaces SET state='reserved'")).rejects.toThrow(/invalid runtime support workspace state transition/);
    await expect(scoped(input.accountId,input.registryTenantId,"DELETE FROM supportv8.runtime_support_workspaces")).rejects.toThrow(/must be tombstoned/);
  ...
  it("keeps real native signup and account binding compatible after account uniqueness removal",async()=>{
    const client=new PostgresClient(undefined,app), signup=new PostgresTenantReservationBackend(client), repository=new ChatRepository(client);
    const tenantId="tenant_native_compat", accountId="acct-shared";
    expect(await signup.reserve({tenantId,tenantSlug:"native-compat.support.test",name:"Native compatibility",operatingMode:"copilot"})).toBe(true);
    await repository.validateAndBindEmailChannel({tenantId,accountId,connectionId:"connection-native",connectorKey:"email.resend.support",recipient:"Native@Support.Test",eventId:"event-native"});
    const tenant=await app.query("SELECT servicev8_account_id FROM supportv8.tenants WHERE id=$1",[tenantId]);
    const binding=await client.withTenantSession(tenantId,db=>db.query<{servicev8_account_id:string;recipient:string}>("SELECT servicev8_account_id,recipient FROM supportv8.email_channel_bindings WHERE tenant_id=$1",[tenantId]));
    expect(tenant.rows).toEqual([{servicev8_account_id:accountId}]);
    expect(binding).toEqual([{servicev8_account_id:accountId,recipient:"native@support.test"}]);
    expect(Number((await app.query("SELECT count(*) FROM supportv8.tenants WHERE servicev8_account_id=$1",[accountId])).rows[0].count)).toBeGreaterThan(1);
